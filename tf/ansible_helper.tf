...
resource "time_sleep" "wait_for_masters_init" {
  depends_on = [aws_instance.masters[0]]
...
resource "null_resource" "provisioner" {
  depends_on    = [
    ...
    time_sleep.wait_for_masters_init,
...
resource "local_file" "ansible_vars_file" {
    content = <<-DOC
        ...
        master_node: ${aws_instance.masters[0].public_dns}
        DOC
